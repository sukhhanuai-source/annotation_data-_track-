...
class LabelmeLoginDialog(QtWidgets.QDialog):
    def __init__(self, api_url: str, parent=None):
        super().__init__(parent)
        self._api_url = api_url
        self.setWindowTitle("Labelme Login")
        self.setModal(True)
        self.setMinimumWidth(380)

        self._username_input = QtWidgets.QLineEdit()
        self._username_input.setPlaceholderText("Username or email")

        self._password_input = QtWidgets.QLineEdit()
        self._password_input.setPlaceholderText("Password")
        self._password_input.setEchoMode(QtWidgets.QLineEdit.Password)

        self._status_label = QtWidgets.QLabel("Login required before opening Labelme.")
    ...
    def _attempt_login(self):
        ...
        # Temporary local login gate for offline use.
        if username == "user" and password == "admin@123":
            os.environ["LABELME_AUTH_USER"] = username
            self.accept()
            return

        self._login_button.setEnabled(False)
        self._status_label.setText("Checking credentials...")
        ...
        try:
            ...
            try:
                ...
                error_body = json.loads(error_raw) if error_raw else {}
            ...
            message = (
                "Cannot reach Django login API.\n"
                "Run server: python /home/hanuai/Downloads/anotation/app/manage.py runserver"
            )
            QMessageBox.critical(self, "Server not reachable", message)
            self._status_label.setText("Django server is not running.")
            self._login_button.setEnabled(True)
            QtWidgets.QApplication.restoreOverrideCursor()
            return
        except Exception as exc:
            QMessageBox.critical(self, "Login error", str(exc))
            self._status_label.setText("Unexpected error.")
            self._login_button.setEnabled(True)
            QtWidgets.QApplication.restoreOverrideCursor()
            return
        ...
        if body.get("success"):
            ...
            os.environ["LABELME_AUTH_USER"] = resolved_username
            self.accept()
            return
...
def require_login_for_labelme() -> bool:
    if os.environ.get("LABELME_SKIP_AUTH") == "1":
        return True
    api_url = os.environ.get("LABELME_AUTH_API", "http://127.0.0.1:8000/api/login/")
    dialog = LabelmeLoginDialog(api_url=api_url)
...
def _get_s3_config() -> dict[str, str]:
    env_candidates = [Path(__file__).resolve().parent.parent / ".env"]
    ...
    return {
        ...
        "region_name": pick("AWS_REGION", "AWS_DEFAULT_REGION", default="ap-south-1"),
        "bucket": pick(
            "S3_BUCKET",
            "AWS_S3_BUCKET",
            "BUCKET_NAME",
...
def _get_s3_client():
    ...
    if not config["aws_access_key_id"] or not config["aws_secret_access_key"]:
        logger.warning("S3 upload skipped: AWS credentials not found in environment or .env")
        return None

    try:
        import boto3
    except Exception:
        logger.warning("S3 upload skipped: boto3 is not installed")
        return None
    ...
    try:
        ...
        logger.warning("S3 client initialization failed: {}", exc)
        return None
    return _S3_CLIENT


def _s3_key_for_annotation(local_json_path: str) -> tuple[str, str]:
    config = _get_s3_config()
    username = os.environ.get("LABELME_AUTH_USER", "unknown_user")
    date_part = datetime.now().strftime("%d%m%Y")
    file_name = osp.basename(local_json_path)
    key = f"{config['root_prefix']}/date_{date_part}/{username}/data/{file_name}"
    return config["bucket"], key


def _upload_annotation_json_to_s3(local_json_path: str) -> bool:
    s3_client = _get_s3_client()
    if s3_client is None:
        return False
    bucket, key = _s3_key_for_annotation(local_json_path)
    try:
        s3_client.upload_file(local_json_path, bucket, key)
    except Exception as exc:
        logger.warning("S3 upload failed for {}: {}", local_json_path, exc)
        return False
    logger.info("Uploaded annotation to s3://{}/{}", bucket, key)
    return True
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        self._video_playback_speed: float = 1.0
        self._video_speed_frame_accumulator: float = 0.0
        ...
        videoPrevFrame = action(
            self.tr("Prev Frame"),
            lambda: self._step_video_frame(-1),
            "Left",
            icon=None,
            tip=self.tr("Previous frame"),
            enabled=False,
        )
        videoPlayPause = action(
            self.tr("Play/Pause"),
            self._toggle_video_play_pause,
            "Space",
            icon=None,
            tip=self.tr("Play or pause video"),
            enabled=False,
        )
        videoNextFrame = action(
            self.tr("Next Frame"),
            lambda: self._step_video_frame(1),
            "Right",
            icon=None,
            tip=self.tr("Next frame"),
            enabled=False,
        )
        save = action(
            self.tr("&Save\n"),
        ...
        self.videoFrameSlider = QtWidgets.QSlider(Qt.Horizontal)
        self.videoFrameSlider.setRange(0, 0)
        self.videoFrameSlider.setMinimumWidth(220)
        self.videoFrameSlider.setEnabled(False)
        self.videoFrameSlider.sliderReleased.connect(self._on_video_slider_released)
        self.videoFrameInfoLabel = QtWidgets.QLabel("Frame: -/-")
        self.videoFrameInfoLabel.setMinimumWidth(120)
        self.videoFrameInfoLabel.setAlignment(Qt.AlignCenter)
        self.videoSpeedCombo = QtWidgets.QComboBox()
        self.videoSpeedCombo.addItems(["0.5x", "1x", "1.5x", "2x", "3x"])
        self.videoSpeedCombo.setCurrentText("1x")
        self.videoSpeedCombo.setEnabled(False)
        self.videoSpeedCombo.currentTextChanged.connect(self._on_video_speed_changed)
        videoSpeedAction = QtWidgets.QWidgetAction(self)
        videoSpeedAction.setDefaultWidget(self.videoSpeedCombo)
        videoSliderAction = QtWidgets.QWidgetAction(self)
        videoSliderAction.setDefaultWidget(self.videoFrameSlider)
        videoFrameInfoAction = QtWidgets.QWidgetAction(self)
        videoFrameInfoAction.setDefaultWidget(self.videoFrameInfoLabel)
...
""",
    ...
    def saveLabels(self, filename):
        ...
        try:
            ...
            other_data = dict(self._other_data or {})
            other_data.setdefault(
                "source_type", "video" if self._video_mode else "image"
            )
            other_data["saved_at_utc"] = _iso_utc_now()
            other_data["annotated_by"] = os.environ.get(
                "LABELME_AUTH_USER", "unknown_user"
            )
            lf.save(
                filename=filename,
                shapes=shapes,
            ...
            uploaded = _upload_annotation_json_to_s3(filename)
            if uploaded:
                self.show_status_message(
                    self.tr("Annotation saved and uploaded to S3."), delay=2000
                )
            else:
                self.show_status_message(
                    self.tr("Annotation saved locally. S3 upload skipped/failed."),
                    delay=2500,
                )
    ...
    def _set_video_controls_enabled(self, enabled: bool) -> None:
        self.actions.videoPrevFrame.setEnabled(enabled)
        self.actions.videoPlayPause.setEnabled(enabled)
        self.actions.videoNextFrame.setEnabled(enabled)
        self.videoFrameSlider.setEnabled(enabled)
        self.videoSpeedCombo.setEnabled(enabled)

    def _video_timer_interval_ms(self) -> int:
        fps = max(self._video_fps, 1.0)
        return max(1, int(1000 / fps))

    def _on_video_speed_changed(self, text: str) -> None:
        try:
            self._video_playback_speed = float(text.lower().replace("x", ""))
        except ValueError:
            self._video_playback_speed = 1.0
        self._video_speed_frame_accumulator = 0.0
        if self._video_is_playing:
            self._video_timer.start(self._video_timer_interval_ms())
        self.show_status_message(
            self.tr("Playback speed: {}").format(text), delay=1000
        )

    def _update_video_frame_info(self) -> None:
        if not self._video_mode:
            self.videoFrameInfoLabel.setText("Frame: -/-")
            return
        self.videoFrameInfoLabel.setText(
            f"Frame: {self._video_current_frame + 1}/{self._video_total_frames}"
        )
        self.videoFrameSlider.blockSignals(True)
        self.videoFrameSlider.setRange(0, max(0, self._video_total_frames - 1))
        self.videoFrameSlider.setValue(self._video_current_frame)
        self.videoFrameSlider.blockSignals(False)

    def _cleanup_video_state(self) -> None:
        ...
        self._video_playback_speed = 1.0
        self._video_speed_frame_accumulator = 0.0
        self.videoSpeedCombo.blockSignals(True)
        self.videoSpeedCombo.setCurrentText("1x")
        self.videoSpeedCombo.blockSignals(False)
        self._set_video_controls_enabled(False)
        self._update_video_frame_info()

    def _video_label_file_path(self, frame_index: int) -> str:
        assert self._video_path is not None
        base_output_dir = self.output_dir or osp.dirname(self._video_path)
        stem = osp.splitext(osp.basename(self._video_path))[0]
        return osp.join(base_output_dir, f"{stem}_frame_{frame_index:06d}.json")

    def _video_metadata(self, frame_index: int) -> dict:
        fps = self._video_fps if self._video_fps > 0 else 0.0
        timestamp_sec = frame_index / fps if fps else 0.0
        return {
            "source_type": "video",
            "video_path": self._video_path,
            "video_file": osp.basename(self._video_path) if self._video_path else None,
            "frame_index": frame_index,
            "fps": fps,
            "timestamp_sec": round(timestamp_sec, 6),
            "timestamp_ms": int(timestamp_sec * 1000),
            "total_frames": self._video_total_frames,
            "annotated_by": os.environ.get("LABELME_AUTH_USER", "unknown_user"),
            "saved_at_utc": _iso_utc_now(),
        }

    def _apply_video_frame(self, frame_bgr, frame_index: int) -> bool:
        if frame_bgr is None or self._video_path is None:
            return False

        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
        h, w, c = frame_rgb.shape
        qimage = QtGui.QImage(
            frame_rgb.data, w, h, c * w, QtGui.QImage.Format_RGB888
        ).copy()
        encoded_ok, encoded_img = cv2.imencode(".jpg", frame_bgr)

        self.resetState()
        self.canvas.setEnabled(False)
        self.imageData = encoded_img.tobytes() if encoded_ok else None
        self.image = qimage
        self.filename = self._video_path
        self._video_current_frame = frame_index
        self.imagePath = f"{self._video_path}#frame_{frame_index:06d}.jpg"
        self._other_data = self._video_metadata(frame_index=frame_index)

        label_file = self._video_label_file_path(frame_index)
        self.labelFile = None
        if QtCore.QFile.exists(label_file) and LabelFile.is_label_file(label_file):
            try:
                ...
                self.errorMessage(
                    ...
                    self.tr("<p><b>%s</b></p><p>Invalid label file.</p>") % e,
                )
                self.labelFile = None
    ...
    def _load_video_file(self, filename: str) -> bool:
        if cv2 is None:
            self.errorMessage(
                self.tr("Video support unavailable"),
                self.tr("OpenCV is required to open videos."),
            )
            return False

        self._cleanup_video_state()
        capture = cv2.VideoCapture(filename)
        if not capture.isOpened():
            self.errorMessage(
                self.tr("Error opening file"),
                self.tr("<p>Could not open video file <b>%s</b>.</p>") % filename,
            )
            return False
        ...
        self._video_fps = float(capture.get(cv2.CAP_PROP_FPS) or 25.0)
        self._video_total_frames = int(capture.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
        ...
        loaded = self._load_video_frame(0, ask_to_continue=False)
        if not loaded:
            self._cleanup_video_state()
            self.errorMessage(
                self.tr("Error opening file"),
                self.tr("<p>Unable to decode the first frame from <b>%s</b>.</p>")
                % filename,
            )
            return False
        if loaded:
            self.show_status_message(
                self.tr("Loaded video %s") % osp.basename(filename), delay=1500
            )
        return loaded

    def _load_video_frame(self, frame_index: int, ask_to_continue: bool = True) -> bool:
        if not self._video_mode or self._video_capture is None or self._video_path is None:
            return False
        if ask_to_continue and not self._can_continue():
            return False

        frame_index = max(0, min(frame_index, self._video_total_frames - 1))
        self._video_capture.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
        ok, frame_bgr = self._video_capture.read()
        if not ok or frame_bgr is None:
            return False
        return self._apply_video_frame(frame_bgr=frame_bgr, frame_index=frame_index)

    def _on_video_timer(self) -> None:
        if not self._video_mode:
            self._video_timer.stop()
            return
        if self._video_current_frame + 1 >= self._video_total_frames:
            self._video_pause()
            return
        assert self._video_capture is not None

        # Advance frames according to selected speed:
        # <1x holds frames across ticks, >1x skips frames.
        self._video_speed_frame_accumulator += max(self._video_playback_speed, 0.1)
        frames_to_advance = int(self._video_speed_frame_accumulator)
        if frames_to_advance <= 0:
            return
        self._video_speed_frame_accumulator -= frames_to_advance

        frame_bgr = None
        advanced = 0
        while advanced < frames_to_advance and (
            self._video_current_frame + advanced + 1
        ) < self._video_total_frames:
            ok, next_frame_bgr = self._video_capture.read()
            if not ok or next_frame_bgr is None:
                self._video_pause()
                return
            frame_bgr = next_frame_bgr
            advanced += 1

        if advanced == 0 or frame_bgr is None:
            self._video_pause()
            return

        next_frame_index = self._video_current_frame + advanced
        self._apply_video_frame(frame_bgr=frame_bgr, frame_index=next_frame_index)

    def _video_play(self) -> None:
        if not self._video_mode:
            return
        if self._is_changed and not self._can_continue():
            return
        self._video_speed_frame_accumulator = 0.0
        self._video_is_playing = True
        self._video_timer.start(self._video_timer_interval_ms())
        self.actions.videoPlayPause.setText(self.tr("Pause"))
        self.canvas.setEnabled(False)

    def _video_pause(self) -> None:
        self._video_is_playing = False
        self._video_timer.stop()
        self.actions.videoPlayPause.setText(self.tr("Play"))
        self.canvas.setEnabled(True)

    def _toggle_video_play_pause(self) -> None:
        if not self._video_mode:
            return
        if self._video_is_playing:
            self._video_pause()
    ...
    def _step_video_frame(self, delta: int) -> None:
        if not self._video_mode:
            return
        self._video_pause()
        self._load_video_frame(self._video_current_frame + delta)

    def _on_video_slider_released(self) -> None:
        if not self._video_mode:
            return
        self._video_pause()
        self._load_video_frame(self.videoFrameSlider.value())
    ...
    def _load_file(self, filename=None):
        ...
        if _is_video_file(filename):
            return self._load_video_file(filename)
    ...
    def _open_file_with_dialog(self, _value: bool = False) -> None:
        ...
        video_formats = [f"*{ext}" for ext in VIDEO_EXTENSIONS]
        filters = self.tr("Image, Video & Label files (%s)") % " ".join(
            formats + video_formats + [f"*{LabelFile.suffix}"]
        )
    ...
    def saveFile(self, _value=False):
        ...
        if self._video_mode:
            self._saveFile(self.getLabelFile())
            return
        if self.labelFile:
    ...
    def getLabelFile(self):
        if self._video_mode:
            return self._video_label_file_path(self._video_current_frame)
        assert self.filename is not None
    ...
    def importDroppedImageFiles(self, imageFiles):
        ...
        for file in imageFiles:
            ...
            if label_file and QtCore.QFile.exists(label_file) and LabelFile.is_label_file(
                label_file
            ):
    ...
    def _import_images_from_dir(
        ...
        filenames = _scan_media_files(root_dir=root_dir)
        if pattern:
            try:
...
def _scan_media_files(root_dir: str) -> list[str]:
    image_extensions: list[str] = [
        f".{fmt.data().decode().lower()}"
        for fmt in QtGui.QImageReader.supportedImageFormats()
    ]
    extensions = image_extensions + list(VIDEO_EXTENSIONS)
    ...
    for root, dirs, files in os.walk(root_dir):
        for file in files:
            if file.lower().endswith(tuple(extensions)):
                ...
                media_files.append(relativePath)

    logger.debug("found {:d} media files in {!r}", len(media_files), root_dir)
    return natsort.os_sorted(media_files)
